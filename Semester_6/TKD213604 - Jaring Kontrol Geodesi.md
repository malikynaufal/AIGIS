...
$$R = \frac{\text{product of non-adjusted sides}}{\text{product of adjusted sides}}$$
...
$$m_{\text{angle}} = \frac{1.5}{\sqrt{2}} \cdot c \; \text{for first-order}$$

where $c$ is the micrometer reading accuracy of the theodolite (typically 0.01").
...
For a Leica TDM6000: $\sigma = \pm(1 \text{ mm} + 0.5 \text{ ppm})$
...
$$\chi^2 = \frac{v^T P v}{\sigma_0^2} \sim \chi^2(n - u)$$

If $\chi^2 > \chi^2_{0.95}(n-u)$, the model may be inadequate or outliers may exist.

### 5.4 Network Types and Unknowns

**Horizontal 2D network:**
- Unknowns: coordinates $(x_i, y_i)$ of $p$ points
- If 2 points fixed: $u = 2(p-2)$

**3D network:**
- Unknowns: $(x_i, y_i, z_i)$
- If 2 points fixed (6 DOF): $u = 3(p-2)$
...
- **Baarda data snooping** test for outlier detection
...
**External reliability:** Impact of undetected blunders on the solution
