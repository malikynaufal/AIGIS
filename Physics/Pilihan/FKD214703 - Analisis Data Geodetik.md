...
$$\text{MLE} = \max_{\theta} \left[-\frac{N}{2}\ln(2\pi) - \frac{1}{2}\ln|\Sigma(\theta)| - \frac{1}{2}(\mathbf{d} - \mathbf{m})^T \Sigma^{-1}(\theta)(\mathbf{d} - \mathbf{m})\right]$$where $\Sigma(\theta) $is the covariance matrix parameterized by noise amplitudes $\theta$.
...
P[k] = \frac{|X[k]|^2}{N \cdot \Delta t} $$Frequency resolution:$\Delta f = 1/(N\Delta t)$.
...
\mathbf{K}_k = \mathbf{P}_{k|k-1}\mathbf{H}_k^T(\mathbf{H}_k\mathbf{P}_{k|k-1}\mathbf{H}_k^T + \mathbf{R}_k)^{-1}\hat{\mathbf{x}}_{k|k} = \hat{\mathbf{x}}_{k|k-1} + \mathbf{K}_k(\mathbf{z}_k - \mathbf{H}_k\hat{\mathbf{x}}_{k|k-1})$$---
...
$$\hat{\mathbf{x}} = (\mathbf{A}^T\mathbf{A} + \lambda^2 \mathbf{I})^{-1}\mathbf{A}^T\mathbf{d} $$The regularization parameter $\lambda $is chosen by L-curve or cross-validation.
