...
A GNSS coordinate time series at station $i$ can be decomposed $x_i(t) = v \cdot t + \sum_{j} \left[a_j \sin(\omega_j t) + b_j \cos(\omega_j t)\right] + \sum_{k} c_k \cdot H(t - t_k) + \epsilon_i(t) $$$
...
$$ P(f) = P_0 f^{-\alpha} $$ where $\alpha = 0$ (white noise),$ \alpha = 1$ (flicker/1-$f$ noise), or $ \alpha = 2$(random walk). Real GNSS series exhibit a mixture. The maximum likelihood estimation (MLE) approach of Langbein (2004) fits $$
...
$The power spectrum is: $$ P[k] = \frac{|X[k]|^2}{N \cdot \Delta t} $$ Frequency resolution:$ \Delta f = 1/(N\Delta t) $.$
...
$$ \mathbf{x}_{k+1} = \mathbf{F}_k \mathbf{x}_k + \mathbf{w}_k, \quad \mathbf{w}_k \sim \mathcal{N}(0, \mathbf{Q}_k)\mathbf{z}_k = \mathbf{H}_k \mathbf{x}_k + \mathbf{v}_k, \quad \mathbf{v}_k \sim \mathcal{N}(0, \mathbf{R}_k
)
...
$Update: $$ \mathbf{K}_k = \mathbf{P}_{k|k-1}\mathbf{H}_k^T(\mathbf{H}_k\mathbf{P}_{k|k-1}\mathbf{H}_k^T + \mathbf{R}_k)^{-1}\hat{\mathbf{x}}_{k|k} = \hat{\mathbf{x}}_{k|k-1} + \mathbf{K}_k(\mathbf{z}_k - \mathbf{H}_k\hat{\mathbf{x}}_{k|k-1})$
