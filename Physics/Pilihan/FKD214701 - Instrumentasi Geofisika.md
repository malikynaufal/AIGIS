...
The seismometer's output voltage $V(\omega) $relates to ground velocity$ \dot{u}(\omega) $via $$ V(\omega) = H(\omega) \cdot \dot{u}(\omega) $$ where the instrument response $$ H(\omega) $is characterized by poles and zeros in the Laplace domain. Typical calibration yields $$
