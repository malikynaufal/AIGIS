...
$$V(\omega) = H(\omega) \cdot \dot{u}(\omega)$$
...
$$H(s) = \frac{A \cdot (s - z_1)(s - z_2) \cdots}{(s - p_1)(s - p_2) \cdots}$$

Correcting raw data requires deconvolution of $H(s)$ — a step handled by the SEED (Standard for the Exchange of Earthquake Data) metadata.
...
$$g = \frac{2(z_2 - z_1)}{(t_2 - t_1)^2}$$

The MICRO-G LaCoste FG5 achieves accuracy $\sigma_g \approx 1\;\mu\text{Gal}$ ($1\;\mu\text{Gal} = 10^{-8}\;\text{m/s}^2$).
...
$$mg = k \cdot l + \text{beam tilt corrections}$$

### 2.2 Tidal Corrections

Gravity observations must be corrected for solid Earth tides (pasut). The tidal acceleration is:

$$g_{\text{tidal}}(t) = \sum_{i} A_i \cos(\omega_i t + \phi_i)$$

with principal tidal constituents $M_2$ (period 12.42 h, amplitude ~50 μGal) and $S_2$ (12.00 h).
...
$$V_{\text{out}}(2f) \propto H_{\text{ext}} \cdot \sin(2\omega t)$$

Sensitivity: typically 0.01 nT; bandwidth: DC–1 kHz. Used widely in ground surveys and satellite missions (e.g., Swarm).

### 3.2 Proton Precession Magnetometer

Exploits Larmor precession of protons in kerosene or water:

$$f_L = \frac{\gamma_p}{2\pi} \cdot |B|$$

where $\gamma_p = 42.577$ MHz/T is the proton gyromagnetic ratio. For Earth's field ($\sim 50\,000$ nT), $f_L \approx 2.13$ kHz. Resolution: ~0.1 nT.
...
| Array | Geometric Factor $K$ | Sensitivity Pattern |
|---|---|---|
