...
$$I = I_0 \left[1 + V\cos\left(\frac{2\pi \cdot \text{OPD}}{\lambda}\right)\right]$$

where $V$ is the visibility (visibilitas) of the fringes and $\lambda$ is the wavelength.
...
Reject if $G > G_{\alpha, N}$ at significance level $\alpha$ with $N$ observations.
...
- **Decision-theoretic**: Minimize total cost $C(T) = C_{\text{cal}}/T + C_{\text{risk}} \cdot P(\text{fail by } T)$
- **Adaptive**: Adjust interval based on latest calibration results
...
$$u_B = \frac{a}{\sqrt{3}} \quad \text{(rectangular distribution)}$$
$$u_B = \frac{a}{\sqrt{6}} \quad \text{(triangular distribution)}$$
$$u_B = \frac{a}{3} \quad \text{(normal distribution, } k=3\text{)}$$
