...
$$\dot{\mathbf{x}}(t) = \mathbf{A}\mathbf{x}(t) + \mathbf{B}\mathbf{u}(t)\mathbf{y}(t) = \mathbf{C}\mathbf{x}(t) + \mathbf{D}\mathbf{u}(t)$$where $\mathbf{x} \in \mathbb{R}^n $is the state vector,$\mathbf{u} \in \mathbb{R}^m $is the input, and $\mathbf{y} \in \mathbb{R}^p $is the output.
...
$$\mathcal{C} = [\mathbf{B} \;\; \mathbf{A}\mathbf{B} \;\; \mathbf{A}^2\mathbf{B} \;\; \cdots \;\; \mathbf{A}^{n-1}\mathbf{B}]$$System is controllable (terkendali) if $\text{rank}(\mathcal{C}) = n$.
...
$$\mathbf{K} = [0 \;\; 0 \;\; \cdots \;\; 1] \cdot \mathcal{C}^{-1} \cdot \alpha_d(\mathbf{A})$$where $\alpha_d(s) = \prod_{i=1}^{n}(s - \lambda_i) $is the desired characteristic polynomial with poles $\lambda_i$.
...
$$\dot{\hat{\mathbf{x}}} = \mathbf{A}\hat{\mathbf{x}} + \mathbf{B}\mathbf{u} + \mathbf{L}(\mathbf{y} - \mathbf{C}\hat{\mathbf{x}})$$The observer gain $\mathbf{L} $is designed so that the estimation error $\mathbf{e} = \mathbf{x} - \hat{\mathbf{x}} $decays
:
...
$$\mathbf{A}^T\mathbf{P} + \mathbf{P}\mathbf{A} = -\mathbf{Q} $$The Lyapunov equation for positive $\mathbf{Q} $has a positive-definite solution $\mathbf{P} $iff all eigenvalues of $\mathbf{A} $have negative real parts.
