...
$$ \text{NF} = 10\log_{10}(F), \quad F = \frac{\text{SNR}_{\text{in}}}{\text{SNR}_{\text{out}}} $$ For cascaded stages, the Friis formula $$ F_{\text{total}} = F_1 + \frac{F_2 - 1}{G_1} + \frac{F_3 - 1}{G_1 G_2} + \cdots $$ This shows the first stage dominates the system noise — the rationale for low-noise amplifiers (LNA) as the front end.
