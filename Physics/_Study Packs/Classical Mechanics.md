...
- $V = V(r)$: gravitational, Coulomb, etc.

For gravity: $V = -GMm/r$.

Lagrangian: $L = \\frac{1}{2}m(\\dot{r}^2 + r^2\\dot{\\theta}^2) + GMm/r$

**Euler-Lagrange for $\\theta$:** $\\frac{d}{dt}(mr^2\\dot{\\theta}) = 0 \\implies \\ell = mr^2\\dot{\\theta} = \\text{const}$ (angular momentum conservation)
...
$H = p\\dot{x} - L = \\frac{p^2}{m} - (\\frac{1}{2}m\\frac{p^2}{m^2} - \\frac{1}{2}kx^2) = \\frac{p^2}{2m} + \\frac{1}{2}kx^2$
...
- $\\{q_i, p_j\\} = \\delta_{ij}$
- $\\{q_i, q_j\\} = 0 = \\{p_i, p_j\\}$
- Hamilton's equations: $\\dot{A} = \\{A, H\\} + \\partial_t A$

If $\\{A, H\\} = 0$, then $A$ is a **constant of motion**.
