...
- **Intensity:** $I = \\langle S \\rangle = \\frac{1}{2}\\epsilon_0 c E_0^2$
...
**Group velocity:** $v_g = c\\sqrt{1 - (f_c/f)^2} < c$

Note: $v_p \\cdot v_g = c^2$ (no information travels faster than $c$).

### Transmission Line (TEM Mode)
$$V(z,t) = V_0^+ e^{-\\gamma z} e^{i(\\omega t - \\beta z)} + V_0^- e^{+\\gamma z} e^{i(\\omega t + \\beta z)}$$

- Propagation constant: $\\gamma = \\alpha + i\\beta = \\sqrt{(R + i\\omega L)(G + i\\omega C)}$
- Characteristic impedance: $Z_0 = \\sqrt{(R + i\\omega L)/(G + i\\omega C)}$
