...
First minimum: $\theta_{\min} \approx 1.22\lambda/D$
...
### Aperture Function $A(x,y)$
For a slit of width $a$:
$$A(x) = \text{rect}(x/a) = \begin{cases}1 & |x| < a/2 \\ 0 & \text{otherwise}\end{cases}$$

### Far-Field Pattern
$$U(f_x, f_y) = \mathcal{F}\{A(x,y)\}$$

where spatial frequencies $f_x = x/(\lambda z)$, $f_y = y/(\lambda z)$.
...
$$\frac{dN_2}{dt} = R_p - \frac{N_2}{\tau_2} - W(N_2 - N_1)$$
$$\frac{d\phi}{dt} = W(N_2 - N_1)\phi - \frac{\phi}{\tau_c}$$
...
- **Spatial coherence:** $A_c \approx (\lambda L/D)^2$
- **Coherence length:** $L_c = c\tau_c = c/\Delta\nu$
