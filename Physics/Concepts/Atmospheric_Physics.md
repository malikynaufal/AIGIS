...
**Stability criterion:** Brunt-Väisälä frequency $N^2 = -\frac{g}{\theta}\frac{d\theta}{dz}$

- $N > 0$: stable stratification (most of troposphere)
...
- Warmer air holds more water vapor: $e_{\text{sat}} \propto \exp(Lv/(RT))$
- H₂O is a strong greenhouse gas (its concentration varies regionally and seasonally)
...
where:
- $T$ = temperature (°C) at height $z$
- $n_e$ = number of electrons along line of sight (slant total electron content, STEC)
- Effect is small: $n \approx 1.0003$ at sea level

### Speed of Light in Air (C-weighted)
$$c_w = \frac{c_0}{n(T, N)}$$
with $n(T, N) = 1 + 776.7 \times 10^{-8} \frac{N}{T + 273.15}$ (approx.)
