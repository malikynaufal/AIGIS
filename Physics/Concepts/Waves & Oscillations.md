...
$$y = 2A\cos\left(\frac{\omega_1 - \omega_2}{2}t\right)\cos\left(\frac{\omega_1 + \omega_2}{2}t\right)$$

Beat frequency:$f_{beat} = |f_1 - f_2| $### Doppler Effec
t

$$f_{obs} = f_0 \frac{v \pm v_{obs}}{v \mp v_{source}} $$

**GNSS application:** Satellite motion shifts the received frequency. For GPS satellite velocity ~3.9 km/s
:
...
$$f(t) = \frac{a_0}{2} + \sum_{n=1}^\infty \left[a_n\cos(n\omega_0 t) + b_n\sin(n\omega_0 t)\right]$$where $\omega_0 = 2\pi/T$:
...
$$R(\tau) = \int s(t) \cdot s(t-\tau)\,dt$$peaks when $\tau $matches the code delay — used for ranging.
