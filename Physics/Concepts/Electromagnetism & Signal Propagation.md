...
$$\Phi = \lambda N + \rho + c\delta_{rx} - c\delta_{sat} - \delta_{iono} + \delta_{trop} + \varepsilon_\Phi$$where $N$is the integer ambiguity (key to centimeter-level positioning).
