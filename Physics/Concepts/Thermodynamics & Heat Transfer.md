...
### Ideal Gas Law$$PV = nRT = Nk_B T$$| Gas | Degrees of freedom$f$|$\gamma = C_P/C_V$|
|-----|----------------------|-------------------|
...
**Heat equation (1D):**$$\frac{\partial T}{\partial t} = \alpha \frac{\partial^2 T}{\partial z^2}$$where thermal diffusivity$\alpha = k/(\rho c_p)$.

### Geothermal Gradient
$$

\frac{dT}{dz} \approx 25-30\ \text{K/km}$$Depth to$100^\circ$C: ~3–4 km
...
\Delta h = \int_0^H \alpha_T(z) \cdot \Delta T(z)\, dz$$-$\alpha_T$ = thermal expansion coefficient (~$10^{-4}$/K near surface)
...
c_p \frac{dT}{dt} = \frac{dq}{dt} - \frac{R_d T}{p}\frac{dp}{dt}$$**Potential temperature:**$$\theta = T\left(\frac{p_0}{p}\right)^{R_d/c_p}$$This is conserved in adiabatic processes — key for atmospheric stability analysis.
