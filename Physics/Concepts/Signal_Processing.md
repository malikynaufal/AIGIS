...
$$P = \lim_{T\to\infty} \frac{1}{T}\int_{-T/2}^{T/2} |f(t)|^2 \, dt \quad [\text{finite, periodic}]$$
...
x[n] = \frac{1}{N}\sum_{k=0}^{N-1} X[k] \, e^{i 2\pi kn/N} $$**Fast Fourier Transform (FFT):**$O(N\log N) $algorithm for DFT computation.
