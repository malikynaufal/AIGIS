...
$$ \gamma_{xy}^2(f) = \frac{|S_{xy}(f)|^2}{S_{xx}(f)S_{yy}(f)} $$ Measures linear correlation in the frequency domain.$ \gamma^2 = 1$: perfect linear relationship.
...
$$ \text{SNR} = \frac{P_{\text{signal}}}{P_{\text{noise}}} = \frac{S_{ss}(f)}{S_{nn}(f)} $$ In dB $$
...
$$ \boxed{f_s \geq 2f_{\text{max}}} $$ If sampling rate is below$ 2f_{\text{max}}$: **aliasing** occurs — frequencies $f > f_s/2 $fold back to $f_s - f$.
...
$$ \text{ENOB} = \frac{\text{SINAD (dB)} - 1.76}{6.02} $$ where SINAD = Signal-to-Noise-and-Distortion ratio.
...
K_k = P_{k|k-1}H_k^T(H_kP_{k|k-1}H_k^T + R_k)^{-1}\hat{x}_{k|k} = \hat{x}_{k|k-1} + K_k(\vec{z}_k - H_k\hat{x}_{k|k-1}) $$ Used in: GNSS navigation solution, PPP ambiguity resolution, inertial navigation integration.
