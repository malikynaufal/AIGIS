...
$$i\hbarrac{\partial \Psi(\mathbf{r}, t)}{\partial t} = \hat{H}\Psi(\mathbf{r}, t)$$

where $\hat{H} = rac{\hat{p}^2}{2m} + V(\mathbf{r})$, $\hat{p} = -i\hbar
abla$
...
This is an **eigenvalue equation** for the Hamiltonian operator.
...
$$[\hat{L}_x, \hat{L}_y] = i\hbar\hat{L}_z \quad 	ext{(cyclic)}$$
...
$$\sigma_x \sigma_p \geq rac{\hbar}{2}$$
$$\sigma_E \sigma_t \geq rac{\hbar}{2}$$

where $\sigma$ = standard deviation of measurement results.
...
$$R_{nl}(r) \propto e^{-r/na_0}\left(rac{2r}{na_0}ight)^l L_{n-l-1}^{2l+1}\left(rac{2r}{na_0}ight)$$
...
- $\hat{\mathbf{S}}^2 = \hbar^2 s(s+1)$
- For electrons: $s = 1/2$, $\hat{S}_z = \pm\hbar/2$
