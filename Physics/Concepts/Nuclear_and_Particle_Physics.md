...
$$2, 8, 20, 28, 50, 82, 126$$Nuclei with magic $Z$and $N $are **doubly magic** ($^{4} $He,$^{16} $O,$^{40} $Ca,$^{208} $Pb).
...
### Exponential Decay La
w

$$N(t) = N_0 e^{-\lambda t} $$where $\lambda$= decay constant,$N_0$= initial number of nuclei.
...
### Gauge Symmetry of the Standard Mode
l
...
$$V(r) = -\frac{4}{3}\frac{\alpha_s}{r} + kr$$where $\alpha_s$= strong coupling constant (~0.1–0.3 at GeV scales),$k \approx 1 $GeV/fm = string tension.
...
f + Z^0 \to f + Z^0$$- Elastic scattering:$\nu_e + e^- \to \nu_e + e^-$(via $Z^0$)
