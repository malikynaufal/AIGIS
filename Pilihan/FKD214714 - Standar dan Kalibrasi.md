...
$$T_{\text{corrected}} = T_{\text{read}} + a_0 + a_1 T_{\text{read}} + a_2 T_{\text{read}}^2$$

Fitting the data: $a_0 = 0.031$, $a_1 = -5.8 \times 10^{-5}$, $a_2 = -2.6 \times 10^{-7}$
...
**Center line**: $\bar{\bar{x}} = \frac{1}{m}\sum_{j=1}^{m}\bar{x}_j$
