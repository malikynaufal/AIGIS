...
$$Y_t = T_t \cdot S_t \cdot R_t \quad \text{(multitipikatif)}$$
...
- Non-seasonal: $(1-\phi_1 B)(1-B)Y_t$
- Seasonal: $(1-\Phi_1 B^{12})(1-B^{12})Y_t$

### 3.4 Prosedur Box-Jenkins
...
$$S(f) = \sum_{k=-\infty}^{\infty} \gamma(k) e^{-i2\pi fk}$$
$$\gamma(k) = \int_{-1/2}^{1/2} S(f) e^{i2\pi fk} \, df$$

### 4.2 Periodogram

$$I(f_j) = \frac{1}{T}\left|\sum_{t=1}^{T} Y_t e^{-i2\pi f_j t}\right|^2, \quad f_j = \frac{j}{T}$$

Properti: $\mathbb{E}[I(f_j)] \approx S(f_j)$ (asymptotically unbiased) tetapi $\text{Var}[I(f_j)] \approx S(f_j)^2$ (**tidak konsisten!**).

### 4.3 Estimator Spektrum Tapered
...
$$\hat{Y}_{T+k|T} \pm 1.96 \cdot \sigma \sqrt{\sum_{j=0}^{k-1} \psi_j^2}$$
...
Estimasi: $\hat{\theta}_1 = -0.65$ (via MLE).

**Langkah 4:** Peramalan:
- $\nabla \hat{Y}_8 = \hat{\theta}_1 \hat{\varepsilon}_7 = (-0.65)(31 - 34.5) = 2.275$
- $\hat{Y}_8 = 31 + 2.275 \approx 33.3$
