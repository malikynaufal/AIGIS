...
The apparent depth of an object at real depth $d$ viewed from above:
...
where $d_o$ is object distance (jarak objek) and $d_i$ is image distance (jarak bayangan). The magnification:
...
$$c_r = 0.0675 \cdot d^2 \;\text{(m)}$$

where $d$ is the sight distance in km. For $d = 100$ m: $c_r = 0.7$ mm.
...
where $(x_0, y_0)$ is the principal point, $f$ is focal length, and $a_i, b_i, c_i$ are rotation matrix elements.
