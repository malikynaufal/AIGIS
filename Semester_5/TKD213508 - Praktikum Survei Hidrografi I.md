...
- **Kalibrasi echo sounder**:
  - Draft calibration: $\Delta d = \text{draft statis} + \text{draft dinamis}$
  - Speed of sound: $c = 1449.2 + 4.6T - 0.055T^2$ (Mackenzie formula)
...
- **Reduksi sounding**: $d_{CD} = d_{\text{observed}} - (\eta_t - \eta_{CD})$

### Modul 5: Data Processing Sederhana
...
- **Koreksi**:
  ...
  3. Draft correction → $d_{\text{net}} = d - \text{static draft}$
- **Interpolasi**: IDW (Inverse Distance Weighting) ke grid raster
