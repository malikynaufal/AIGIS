...
1. $0 \\in W$
2. $u, v \\in W$ → $u+v \\in W$
3. $v \\in W$, $c \\in F$ → $cv \\in W$

**Span**: $\\text{span}\\{v_1,\\ldots,v_k\\} = \\{\\sum c_i v_i : c_i \\in F\\}$

#### 1.3 Linear Independence
$\\{v_1,\\ldots,v_k\\}$ is linearly independent if $\\sum c_i v_i = 0$ → $c_1 = \\cdots = c_k = 0$.
...
- $\\text{Ker}(T) = \\{v: Tv=0\\}$
- $\\text{Im}(T) = \\{Tv: v \\in V\\}$
- **Rank-Nullity**: $\\dim(\\text{Ker}(T)) + \\dim(\\text{Im}(T)) = \\dim(V)$
...
$P_{B\\leftarrow C}$ = transition matrix from basis $C$ to basis $B$.
...
**Pythagorean**: If $u \\perp v$, $\\|u+v\\|^2 = \\|u\\|^2 + \\|v\\|^2$

#### 3.4 Gram-Schmidt Process
Given $\\{v_1,\\ldots,v_k\\}$, produce orthonormal $\\{u_1,\\ldots,u_k\\}$:

$$u_1 = \\frac{v_1}{\\|v_1\\|}$$
$$w_k = v_k - \\sum_{i=1}^{k-1} \\langle v_k,u_i\\rangle u_i, \\quad u_k = \\frac{w_k}{\\|w_k\\|}$$

#### 3.5 Orthogonal Complement

$W^\\perp = \\{v: \\langle v,w\\rangle = 0 \\, \\forall w \\in W\\}$

$\\dim(W) + \\dim(W^\\perp) = \\dim(V)$

### Weeks 7-8: Least Squares
...
For $A\\hat{x} \\approx b$: $A^TA\\hat{x} = A^Tb$

When $A$ has full column rank: $\\hat{x} = (A^TA)^{-1}A^Tb$

#### 4.3 Orthogonal Projection Matrix

$P_W = A(A^TA)^{-1}A^T$ (projects onto $\\text{Col}(A)$)
...
For symmetric $A$: $A = Q\\Lambda Q^T$ with $Q$ orthogonal.

$A = \\sum_{i=1}^n \\lambda_i q_i q_i^T$

#### 5.4 Quadratic Forms

$Q(x) = x^TAx$ — sign determined by eigenvalues.
...
Used for: pseudoinverse, low-rank approximation, condition number.
...
$A = LU$ — Gaussian elimination factorization.
