...
- **Axioms of probability** (Kolmogorov):
  1. $0 \\leq P(A) \\leq 1$
  2. $P(\\Omega) = 1$
  3. Countable additivity: $P(\\cup_i A_i) = \\sum_i P(A_i)$ for disjoint events
...
- **Expected value**: $E[X] = \\sum x_i p_i$ or $\\int xf(x)dx$
- **Variance**: $\\text{Var}(X) = E[X^2]-(E[X])^2$
- **Covariance and correlation**: $\\text{Cov}(X,Y)$, $\\rho_{XY}$
...
- Chi-square($n$): $\\sum Z_i^2$

### Unit 5: Limit Theorems

- **Law of Large Numbers** (weak and strong): $\\bar{X}_n \\to \\mu$
- **Central Limit Theorem**: $\\frac{\\bar{X}-\\mu}{\\sigma/\\sqrt{n}} \\xrightarrow{d} N(0,1)$

### Unit 6: Joint Distributions

- **Joint PMF/PDF**: $p(x,y)$ or $f(x,y)$
- **Marginals**: $p_X(x) = \\sum_y p(x,y)$ or $f_X(x) = \\int f(x,y)dy$
...
A GPS station has 90% probability of correct signal. Signal noise test is 80% sensitive. What's the probability of a clean signal given a positive noise test?

$P(\\text{clean}) = 0.9$, $P(\\text{positive}|\\text{clean}) = 0.8$
$P(\\text{positive}|\\text{noisy}) = 0.3$

$$P(\\text{clean}|+) = \\frac{0.8 \\times 0.9}{0.8\\times 0.9 + 0.3\\times 0.1} = \\frac{0.72}{0.75} = 0.96$$
