...
where $I(\theta) = E\left[\left(\frac{\partial}{\partial \theta} \log f(X;\theta)\right)^2\right]$ is the **Fisher information**.
...
**Score equation:** $\frac{\partial \ell}{\partial \theta} = 0$
...
$$\frac{\partial \ell}{\partial \sigma^2} = -\frac{n}{2\sigma^2} + \frac{1}{2(\sigma^2)^2}\sum(x_i - \mu)^2 = 0 \implies \hat{\sigma}^2 = \frac{1}{n}\sum(x_i - \bar{x})^2$$
...
The error ellipse at confidence level $P$ has semi-axes:

$$a, b = \sqrt{\lambda_{1,2} \cdot \chi^2_{2,P}}$$
