...
$$p_X(k) = P(X = k)$$Properties:$0 \leq p_X(k) \leq 1$,$\sum_k p_X(k) = 1$### 5.3 Probability Density Function (PDF) — Continuou
s

$$P(a < X < b) = \int_a^b f_X(x)\, dx$$

Properties:$f_X(x) \geq 0$,$\int_{-\infty}^{\infty} f_X(x)\, dx = 1$### 5.4 Cumulative Distribution Function (CDF)$$F_X(x) = P(X \leq x) = \begin{cases}
...
E[Y] = E[E[Y|X]]\text{Var}(Y) = \text{Var}(E[Y|X]) + E[\text{Var}(Y|X)]$$---
