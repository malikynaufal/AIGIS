...
Properties: $ 0 \leq p_X(k) \leq 1 $,$\sum_k p_X(k) = 1 $### 5.3 Probability Density Function (PDF) — Continuou
s

$ $ P(a < X < b) = \int_a^b f_X(x)\, dx $$

Properties: $ f_X(x) \geq 0 $,$\int_{-\infty}^{\infty} f_X(x)\, dx = 1 $### 5.4 Cumulative Distribution Function (CDF)
...
Properties: non-decreasing,$\lim_{x\to-\infty}F(x)=0 $,$\lim_{x\to\infty}F(x)=1 $### 5.5 Moments
