...
y' + P(x)y = Q(x)\mu(x) = e^{\int P(x)\,dx}y = \frac{1}{\mu(x)} \left[\int \mu(x)Q(x)\,dx + C\right
]
...
A system $\mathbf{y}' = A\mathbf{y} $has solution
:

$$\mathbf{y}(t) = e^{At}\mathbf{y}(0)$$where $e^{At} $is the matrix exponential.
...
$$\ddot{r} - r\dot{\theta}^2 = -\frac{GM}{r^2}r\ddot{\theta} + 2\dot{r}\dot{\theta} = 0$$From the second, angular momentum $h = r^2\dot{\theta} $is conserved. Substituting into the first gives the orbit equation
:

$$r(t) = \frac{a(1-e^2)}{1 + e\cos\theta} $$where $a$= semi-major axis,$e$= eccentricity — an ellipse!
...
$$\dot{\mathbf{x}} = F\mathbf{x} + G\mathbf{w}\dot{P} = FP + PF^T + Q$$where $F$= dynamics matrix,$Q$= process noise,$P$= state covariance.
