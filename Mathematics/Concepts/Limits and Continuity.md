...
\lim_{x \to a} f(x) = L$$if for every$\varepsilon > 0$, there exists a $\delta > 0$such that:$$0 < |x - a| < \delta \implies |f(x) - L| < \varepsilon$$| Symbol | Meaning |
|--------|---------|
...
### One-Sided Limits$$\lim_{x \to a^-} f(x) = L^- \quad \text{(left-hand limit)}
$$

$$
\lim_{x \to a^+} f(x) = L^+ \quad \text{(right-hand limit)}$$The two-sided limit exists **iff**$L^- = L^+$.
...
3.$\lim_{x \to a} f(x) = f(a)$### Types of Discontinuities
...
- [[Multivariable Calculus]] — Limits in $\mathbb{R}^n$: $\lim_{\mathbf{x} \to \mathbf{a}} f(\mathbf{x}) = L$- [[Numerical Methods]] — Finite differences approximate limits
