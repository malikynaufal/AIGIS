...
Minimise $\mathbf{v}^TP\mathbf{v} $where $P $is weight matrix:

$$A^TPA\hat{\mathbf{x}} = A^TPl\hat{\mathbf{x}} = (A^TPA)^{-1}A^TPl$$### 2.3 Solution Properties
...
$$\hat{\sigma}_0^2 = \frac{\mathbf{v}^TP\mathbf{v}}{n-u} $$where $n-u$= degrees of freedom.
...
Linearize: $\mathbf{f}(\mathbf{x} + \delta\mathbf{x}) \approx \mathbf{f}(\mathbf{x}) + J\delta\mathbf{x} $Iterate:

$$J^TJ\delta\hat{\mathbf{x}} = J^T(\mathbf{l} - \mathbf{f}(\mathbf{x}))\mathbf{x}_{k+1} = \mathbf{x}_k + \delta\hat{\mathbf{x}} $$### 5.2 Levenberg-Marquardt
...
$$\hat{\mathbf{x}} = (A^TA + \alpha I)^{-1}A^T\mathbf{l} $$Penalises large coefficients; improves stability for ill-conditioned $A$.
