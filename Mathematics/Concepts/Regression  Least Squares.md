...
where $\varepsilon \sim N(0, \sigma^2)$ is noise.
...
### 4.1 Cholesky Approach

$$P = L^TL$$

Transform: $\tilde{A} = LA$, $\tilde{l} = Ll$

Solve $\tilde{A}^T\tilde{A}\hat{\mathbf{x}} = \tilde{A}^T\tilde{l}$
...
Linearize: $\mathbf{f}(\mathbf{x} + \delta\mathbf{x}) \approx \mathbf{f}(\mathbf{x}) + J\delta\mathbf{x}$

Iterate:
$$J^TJ\delta\hat{\mathbf{x}} = J^T(\mathbf{l} - \mathbf{f}(\mathbf{x}))$$
$$\mathbf{x}_{k+1} = \mathbf{x}_k + \delta\hat{\mathbf{x}}$$

### 5.2 Levenberg-Marquardt

Adds damping parameter $\lambda$:
$$(J^TJ + \lambda I)\delta\hat{\mathbf{x}} = J^T(\mathbf{l} - \mathbf{f})$$

- $\lambda$ large: gradient descent (stable)
- $\lambda$ small: Gauss-Newton (fast convergence)

## 6. Hypothesis Testing in Regression

### 6.1 t-Test for Individual Coefficients

$$t = \frac{\hat{\beta}_j}{\text{SE}(\hat{\beta}_j)} \sim t_{n-p}$$

### 6.2 F-Test for Model Significance
...
- **Residual plots**: $\hat{v}_i$ vs $\hat{y}_i$ or $x_i$
