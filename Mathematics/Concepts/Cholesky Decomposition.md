...
A = G G^T$$where$G$= lower triangular,$A$= symmetric positive definite.
...
## In Geodesy
The normal matrix$N = H^T P H$is always symmetric positive definite. Cholesky decomposition is the standard method for solving:$$N\hat{x} = b \quad \rightarrow \quad \hat{x} = G^{-T}G^{-1}b
$$
