...
For any population with mean $\mu $and finite variance $\sigma^2$, the sample mean $\bar{X} $of $n $independent observations has
:
...
$$\text{SE}(\bar{x}) = \frac{\sigma}{\sqrt{n}} \approx \frac{s}{\sqrt{n}} $$This is the standard deviation of the sampling distribution of $\bar{x} $— it measures how precisely the sample mean estimates the population mean.
...
$$n = \left(\frac{z_{\alpha/2} \cdot \sigma}{\text{Margin of Error } E}\right)^2$$**Example:** Desired half-width $E = \pm 2 $mm,$\sigma \approx 10 $mm, 95% confidence
:
