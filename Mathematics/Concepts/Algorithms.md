...
### Binary Search (Pencarian Biner) —$O(\log n)$Requires sorted input (input harus terurut). Repeatedly halves the search interval:$$\text{mid} = \left\lfloor \frac{\text{low} + \text{high}}{2} \right\rfloor$$```python
...
Numerically more stable than normal equations:$$\mathbf{A} = \mathbf{Q}\mathbf{R} \implies \hat{\mathbf{x}} = \mathbf{R}^{-1}\mathbf{Q}^T\mathbf{L}$$## Spatial / Geospatial Algorithms (Algoritma Spasial)
