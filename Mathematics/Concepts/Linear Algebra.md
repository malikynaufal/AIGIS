...
### 3.1 Definition (2×2)$$\det\begin{bmatrix} a & b \\ c & d \end{bmatrix} = ad - bc$$### 3.2 Cofactor Expansion$$\det(A) = \sum_{j=1}^n (-1)^{i+j} a_{ij}M_{ij}$$where$M_{ij}$is the$(n-1)\times(n-1)$minor.
...
### 5.1 LU Decomposition$$A = LU$$where$L$is lower triangular,$U$is upper triangular.
...
### 5.3 Singular Value Decomposition (SVD)$$A = U\Sigma V^T$$where:
...
A^TA\hat{\mathbf{x}} = A^T\mathbf{b}$$### 6.2 Moore-Penrose Pseudoinverse$$A^+ = V\Sigma^+U^T$$where$\Sigma^+$has reciprocals of nonzero singular values.$$\hat{\mathbf{x}} = A^+\mathbf{b}$$## 7. Practice Problems

### Problem 1
Find eigenvalues of$A = \begin{bmatrix} 2 & 1 \\ 1 & 2 \end{bmatrix}$**Solution**:$$\det(A-\lambda I) = (2-\lambda)^2 - 1 = \lambda^2 - 4\lambda + 3 = (\lambda-1)(\lambda-3) = 0
$$
