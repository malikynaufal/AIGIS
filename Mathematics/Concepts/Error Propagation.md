...
\mathbf{Q}_{yy} = \mathbf{J} \mathbf{Q}_{xx} \mathbf{J}^T$$Where $\mathbf{Q} = \sigma_0^{-2} \cdot \mathbf{C} $is the **cofactor matrix** (without unit-weight scale).
...
d = \sqrt{(x_2 - x_1)^2 + (y_2 - y_1)^2}\sigma_d^2 = \mathbf{J}\mathbf{C}_{\hat{x}\hat{x}}\mathbf{J}^T$$where $\mathbf{J} $contains partial derivatives $\partial d/\partial x_1, \partial d/\partial y_1, \partial d/\partial x_2, \partial d/\partial y_2$.
...
\sigma_H = \sigma \sqrt{n} = \epsilon \sqrt{L} $$where $\sigma$= standard deviation per setup,$n$= number of setups,$\epsilon$= standard deviation per km,$L$= line length in km.
