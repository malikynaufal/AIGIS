...
$$ J = \begin{bmatrix} -(N+h)\sin\phi\cos\lambda & -(N+h)\cos\phi\sin\lambda & \cos\phi\cos\lambda \\ -(N+h)\sin\phi\sin\lambda & (N+h)\cos\phi\cos\lambda & \cos\phi\sin\lambda \\ (N(1-e^2)+h)\cos\phi & 0 & \sin\phi \end{bmatrix}+ \text{ terms with } \partial N/\partial\phi $$ This Jacobian is essential for error propagation from geodetic to ECEF coordinates.
...
U(\mathbf{r}) = G \int \frac{\rho(\mathbf{r}')}{|\mathbf{r} - \mathbf{r}'|}\, dV'$$ This requires multivariable integration — spherical harmonic decomposition simplifies it.
