...
$$ R_n(x) = \frac{f^{(n+1)}(\xi)}{(n+1)!}(x-a)^{n+1}$$ where $\xi $is between$ a $and $ x $.
...
$$ f(x) \approx f(x_0) + f'(x_0)(x-x_0) + \frac{f''(x_0)}{2}(x-x_0)^2 $$ Minimizing:$ f'(x_0) + f''(x_0)(x-x_0) = 0 $gives:$$ x = x_0 - \frac{f'(x_0)}{f''(x_0)}$$ This is Newton's method for optimization.
...
$$ f(\mathbf{x}) \approx f(\mathbf{a}) + \nabla f^T \Delta\mathbf{x} + \frac{1}{2} \Delta\mathbf{x}^T H \Delta\mathbf{x}$$ where $ H $ is the Hessian matrix.
