...
$$\int_a^b f(x)\,dx = F(b) - F(a)$$This represents the signed area under $f(x) $from $x = a $to $x = b$.
...
\int_a^b f(x)\,dx = F(b) - F(a)$$**Proof of FTC Part 1**: Let $F(x) = \int_a^x f(t)\,dt$. Then:
...
\int u\,dv = uv - \int v\,du$$**Strategy**: Choose $u $using LIATE (Logs, Inverse trig, Algebraic, Trig, Exponential).
