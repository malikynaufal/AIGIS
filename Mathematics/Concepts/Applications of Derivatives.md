...
$$\text{If } f'(c) = 0 \text{ and } f''(c) > 0 \implies f \text{ has a local minimum at } c\text{If } f'(c) = 0 \text{ and } f''(c) < 0 \implies f \text{ has a local maximum at } c\text{If } f''(c) = 0 \implies \text{test is inconclusive (uji tidak tegas)} $$### Constrained Optimization (Optimasi Terkonstraksi)
...
$$S(\mathbf{x}) = \mathbf{v}^T \mathbf{P} \mathbf{v} = (\mathbf{A}\mathbf{x} - \mathbf{L})^T \mathbf{P} (\mathbf{A}\mathbf{x} - \mathbf{L})$$subject to observation equations. Setting $\frac{\partial S}{\partial \mathbf{x}} = \mathbf{0} $yields the **normal equations**:
