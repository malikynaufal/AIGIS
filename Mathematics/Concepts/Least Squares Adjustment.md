...
\hat{\mathbf{x}} = (\mathbf{H}^T\mathbf{P}\mathbf{H})^{-1}\mathbf{H}^T\mathbf{P}\mathbf{y} $$### Nonlinear Model (Gauss-Newton Iteration)

For $\mathbf{f}(\mathbf{x}) $nonlinear, linearize around current estimate $\mathbf{x}_k$:

$$\mathbf{y} \approx \mathbf{f}(\mathbf{x}_k) + \mathbf{J}_k \Delta\mathbf{x}\Delta\mathbf{x}_k = (\mathbf{J}_k^T\mathbf{P}\mathbf{J}_k)^{-1}\mathbf{J}_k^T\mathbf{P}\mathbf{r}_k$$where $\mathbf{J}_k = \partial\mathbf{f}/\partial\mathbf{x}|_{\mathbf{x}_k} $and $\mathbf{r}_k = \mathbf{y} - \mathbf{f}(\mathbf{x}_k)$.

Iterate until $\ |\Delta\mathbf{x}\| < \epsilon$.

### Least Squares with Constraints

**Equality constraints** (Helmert): $\mathbf{C}\mathbf{x} = \mathbf{w} $Via Lagrange multipliers
:
