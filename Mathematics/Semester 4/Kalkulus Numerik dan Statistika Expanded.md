...
**Example**: Find $\\sqrt{2}$ using Newton: $x_{n+1} = \\frac{1}{2}(x_n + \\frac{2}{x_n})$
...
- **Trapezoidal rule**: $\\int_a^b f(x)dx \\approx \\frac{h}{2}[f(a)+2\\sum f(x_i)+f(b)]$
- **Simpson's rule**: $\\int_a^b f(x)dx \\approx \\frac{h}{3}[f(a)+4\\sum f(\\text{odd})+2\\sum f(\\text{even})+f(b)]$
