...
**Key Limits**:$$\\lim_{x \\to 0} \\frac{\\sin x}{x} = 1, \\quad \\lim_{x \\to 0} \\frac{e^x - 1}{x} = 1$$#### 1.3 Continuity$f$is continuous at$a$if:
1.$f(a)$exists
...
\\exists c \\in (a,b): f'(c) = \\frac{f(b)-f(a)}{b-a}$$#### 3.3 First Derivative Test
...
#### 3.7 Newton's Method$$x_{n+1} = x_n - \\frac{f(x_n)}{f'(x_n)}$$#### 3.8 L'Hôpital's Rule
For$\\frac{0}{0}$or$\\frac{\\infty}{\\infty}$:
$$

\\lim \\frac{f}{g} = \\lim \\frac{f'}{g'}$$### Weeks 9-12: Integrals

#### 4.1 Antiderivatives$F' = f$→$F$is antiderivative of$f$.
General antiderivative: $F(x) + C$#### 4.2 Indefinite Integrals$$\\int f(x)\\,dx = F(x) + C$$Basic formulas:$\\int x^n = x^{n+1}/(n+1)$, $\\int e^x = e^x$, $\\int \\sin x = -\\cos x$, etc.

#### 4.3 Riemann Sums and Definite Integrals
$$

\\int_a^b f(x)\\,dx = \\lim_{n \\to \\infty} \\sum_{i=1}^n f(x_i^*)\\Delta x$$#### 4.4 Fundamental Theorem of Calculus
**Part 1**:$\\frac{d}{dx}\\int_a^x f(t)\\,dt = f(x)$**Part 2**:$\\int_a^b f(x)\\,dx = F(b) - F(a)$where$F' = f$#### 4.5 Substitution Rule$$\\int f(g(x))g'(x)\\,dx = \\int f(u)\\,du \\quad (u = g(x))$$### Weeks 13-14: Applications of Integrals

#### 5.1 Area Between Curves$$A = \\int_a^b |f(x) - g(x)|\\,dx$$#### 5.2 Volumes
**Disk**:$V = \\pi\\int_a^b [R(x)]^2\\,dx$**Washer**:$V = \\pi\\int_a^b ([R(x)]^2 - [r(x)]^2)\\,dx$**Shell**:$V = 2\\pi\\int_a^b x\\cdot f(x)\\,dx$#### 5.3 Work$$W = \\int_a^b F(x)\\,dx$$---
...
\\lim_{x \\to 0} \\frac{3x}{5x} \\cdot \\frac{\\sin 3x}{3x} \\cdot \\frac{5x}{\\sin 5x} = \\frac{3}{5} \\cdot 1 \\cdot 1 = \\frac{3}{5}$$### Example 2: Optimization
Find the dimensions of a cylinder with volume 1000 cm³ that minimizes surface area.

**Solution**:$V = \\pi r^2 h = 1000$, so $h = 1000/(\\pi r^2)$
$S = 2\\pi r^2 + 2\\pi r h = 2\\pi r^2 + 2000/r$
$S' = 4\\pi r - 2000/r^2 = 0$→$r^3 = 500/\\pi$→$r \\approx 5.42$cm$h = 1000/(\\pi r^2) \\approx 10.84$cm

### Example 3: Definite Integral
Evaluate$\\int_0^{\\pi/2} \\sin^3 x \\cos^2 x\\,dx$.
...
$= -\\frac{\\cos^3 x}{3} + \\frac{\\cos^5 x}{5}$From 0 to$\\pi/2$: $[0] - [-\\frac{1}{3} + \\frac{1}{5}] = \\frac{1}{3} - \\frac{1}{5} = \\frac{2}{15}$---
...
| Topic | Formula |
|-------|---------|
| Derivative def. |$f'(x) = \\lim_{h\\to0}(f(x+h)-f(x))/h$|
