...
$$ where $ C $ is the **constant of integration** (arbitrary constant).
...
$$ This connects the antiderivative to the definite integral and provides the practical method for evaluation.
...
$$ **Example:**$$\int \frac{x}{1+x^2}\, dx \quad \text{let } u = 1+x^2, \, du = 2x\, dx= \frac{1}{2}\int \frac{du}{u} = \frac{1}{2}\ln|1+x^2| + C
...
$$ **Example:**$$\int \frac{5x+3}{(x+1)(x+2)}\, dx = \int\left(\frac{-2}{x+1} + \frac{7}{x+2}\right)dx = -2\ln|x+1| + 7\ln|x+2| + C
...
$$\int \frac{dx}{\sqrt{4-x^2}} \quad x = 2\sin\theta,\; dx = 2\cos\theta\, d\theta= \int \frac{2\cos\theta}{2\cos\theta}\, d\theta = \theta + C = \arcsin\frac{x}{2} + C
...
$$ where $ x_i $are random points in $\Omega $. Error: $O(1/\sqrt{N})$.
