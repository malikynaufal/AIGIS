...
$$\min_{\mathbf{x}} \, \phi = \mathbf{v}^T\mathbf{P}\mathbf{v} $$where $\mathbf{v} = \mathbf{l} - \mathbf{f}(\hat{\mathbf{x}}) $is the residual vector and $\mathbf{f}(\hat{\mathbf{x}}) $is the functional model.
...
$$\mathbf{B}\mathbf{v} = \mathbf{w} $$where $\mathbf{B} $is the condition matrix and $\mathbf{w} $is the misclosure vector. The least-squares solution
:
