...
### 2.4 Solving Systems $A\\mathbf{x} = \\mathbf{b}$
...
$$r_{ij} = \\mathbf{q}_i^T\\mathbf{a}_j, \\quad r_{jj} = \\|\\text{orthogonal component}\\|$$
...
$r_{jj} = \\|\\mathbf{v}_j\\|$; $\\quad \\mathbf{q}_j = \\mathbf{v}_j / r_{jj}$

### 3.4 Householder Reflections (Preferred)
...
$A - \\sigma I$ → QR → $Q_{k+1}R_{k+1}$ → $A_{k+1} = R_{k+1}Q_{k+1} + \\sigma I$
...
**Solution**:
$x^{(1)} = (7-0)/4 = 7/4 = 1.75$
$y^{(1)} = (5-0)/3 = 5/3 \\approx 1.667$

Second step:
$x^{(2)} = (7-5/3)/4 = (16/3)/4 = 4/3 \\approx 1.333$
$y^{(2)} = (5-7/4)/3 = (13/4)/3 \\approx 1.083$
...
$A^+ = \\begin{bmatrix} 1/3 & 0 \\\\ 0 & 1/2 \\end{bmatrix}$
