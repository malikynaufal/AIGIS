...
\\int u\\,dv = uv - \\int v\\,du$$**LIATE Priority**: Logarithms > Inverse trig > Algebraic > Trig > Exponential

**Example**:$\\int x e^x\\,dx$: $u=x$, $dv=e^x dx$→$xe^x - e^x + C$**Reduction formulas**:$$\\int \\sin^n x\\,dx = -\\frac{\\sin^{n-1}x\\cos x}{n} + \\frac{n-1}{n}\\int \\sin^{n-2}x\\,dx$$#### 1.2 Trigonometric Integrals
...
$x = 2\\tan\\theta$, $dx = 2\\sec^2\\theta\\,d\\theta$, $\\sqrt{4+x^2} = 2\\sec\\theta$
$$

\\int \\frac{2\\sec^2\\theta\\,d\\theta}{4\\tan^2\\theta \\cdot 2\\sec\\theta} = \\int \\frac{\\sec\\theta\\,d\\theta}{4\\tan^2\\theta} = \\frac{1}{4}\\int \\frac{\\cos\\theta}{\\sin^2\\theta}\\,d\\theta = -\\frac{1}{4\\sin\\theta}+C = -\\frac{\\sqrt{4+x^2}}{4x}+C$$#### 1.4 Partial Fractions
...
= -2\\ln|x| + \\frac{1}{x} + 2\\ln|x-1| + C$$### Weeks 4-5: Improper Integrals

#### 2.1 Infinite Integrals$$\\int_1^{\\infty} \\frac{1}{x^p}\\,dx = \\begin{cases} \\frac{1}{p-1} & p>1 \\\\ \\text{diverges} & p\\leq 1 \\end{cases}$$**Comparison test**: If$0 \\leq f(x) \\leq g(x)$and$\\int g$converges →$\\int f$converges.

#### 2.2 Discontinuous Integrands
$$

\\int_0^1 \\frac{1}{\\sqrt{x}}\\,dx = [2\\sqrt{x}]_0^1 = 2$$(convergent)
$$

\\int_0^1 \\frac{1}{x}\\,dx = [\\ln|x|]_0^1 = \\infty$$(divergent)
...
#### 3.2 Series$$\\sum_{n=1}^{\\infty} a_n = \\lim_{N\\to\\infty} S_N, \\quad S_N = \\sum_{n=1}^N a_n$$**Nth term test**: If$\\lim a_n \\neq 0$, the series diverges.

#### 3.3 Geometric Series
$$

\\sum_{n=0}^{\\infty} ar^n = \\frac{a}{1-r} \\quad (|r|<1)$$**Diverges** when$|r| \\geq 1$.

#### 3.4 Telescoping Series
$$

\\sum_{n=1}^{\\infty}\\left(\\frac{1}{n}-\\frac{1}{n+1}\\right) = 1$$#### 3.5 p-Series$$\\sum_{n=1}^{\\infty} \\frac{1}{n^p} \\begin{cases} \\text{converges} & p>1 \\\\ \\text{diverges} & p\\leq 1 \\end{cases}$$#### 3.6 Comparison Tests
...
\\sum a_n \\text{ and } \\int_1^{\\infty} f(x)\\,dx \\text{ both converge or both diverge}$$#### 4.2 Ratio Test$$\\rho = \\lim_{n\\to\\infty} \\frac{a_{n+1}}{a_n}$$-$\\rho < 1$: converges
...
\\rho = \\lim_{n\\to\\infty} |a_n|^{1/n}$$Same rules as ratio test.
...
#### 5.1 Power Series$$\\sum_{n=0}^{\\infty} c_n(x-a)^n$$**Radius of convergence**:$R = 1/\\lim|c_{n+1}/c_n|$(ratio test)
...
f(x) = \\sum_{n=0}^{\\infty} \\frac{f^{(n)}(a)}{n!}(x-a)^n$$#### 5.3 Common Taylor Series
...
#### 5.4 Taylor's Theorem with Remainder$$f(x) = T_n(x) + R_n(x)$$where$T_n$is the$n$th degree Taylor polynomial and:
$$
...
Using series to evaluate limits:$$\\lim_{x\\to 0}\\frac{e^x-1-x}{x^2} = \\lim_{x\\to 0}\\frac{(1+x+x^2/2+\\cdots)-1-x}{x^2} = \\lim_{x\\to 0}\\frac{x^2/2+\\cdots}{x^2} = \\frac{1}{2}$$#### 6.3 Definite Integrals$$\\int e^{-x^2}\\,dx = \\int\\left(1-x^2+\\frac{x^4}{2!}-\\cdots\\right)dx = x - \\frac{x^3}{3}+\\frac{x^5}{10}-\\cdots$$---
...
= \\frac{3}{4}\\ln|x+3|+\\frac{5}{4}\\ln|x-1|+C$$### Example 2: Alternating Series Error
