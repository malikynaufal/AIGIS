...
Existence and Uniqueness Theorem (Picard-Lindelöf): If $f$ and $\\partial f/\\partial y$ are continuous near $(x_0, y_0)$, then $y' = f(x,y)$, $y(x_0) = y_0$ has a unique local solution.
...
$M(x,y)\\,dx + N(x,y)\\,dy = 0$ is exact if $M_y = N_x$.
...
Substitute $v = y^{1-n}$ → linear in $v$.

**Example**: $y' + y/x = y^2/x^2$ ($n=2$)
$v = y^{-1}$: $v' - v/x = -1/x^2$ → linear, $\\mu = 1/x$
$(v/x)' = -1/x^3$ → $v/x = 1/(2x^2) + C$ → $v = 1/(2x) + Cx$

#### 1.6 Homogeneous Equations

$y' = F(y/x)$ → substitute $v = y/x$, $y = xv$, $y' = v + xv'$

### Weeks 4-6: Second-Order Linear ODEs

#### 2.1 Homogeneous: $y'' + py' + qy = 0$
...
Characteristic: $r^2-3r+2 = (r-1)(r-2) = 0$
...
#### 2.3 Nonhomogeneous: $y'' + py' + qy = g(x)$
...
$A = \\begin{bmatrix}1&1\\\\4&1\\end{bmatrix}$, $\\lambda^2-2\\lambda-3=0$, $\\lambda_1=3$, $\\lambda_2=-1$

Eigenvectors: $\\mathbf{v}_1=(1,2)^T$, $\\mathbf{v}_2=(1,-2)^T$
...
$\\mathcal{L}^{-1} = -2e^{-t} + 7e^{-2t}$

#### 4.4 Solving ODEs with Laplace
...
1. Solve $y' = y^2$, $y(0) = 1/3$
2. Solve $y'' + 4y' + 4y = e^{-2x}$
3. Solve the IVP: $\\mathbf{x}' = \\begin{bmatrix}1&2\\\\-1&1\\end{bmatrix}\\mathbf{x}$, $\\mathbf{x}(0)=\\begin{bmatrix}1\\\\0\\end{bmatrix}$
4. Use Laplace to solve $y''-3y'+2y=\\sin t$, $y(0)=0$, $y'(0)=1$
