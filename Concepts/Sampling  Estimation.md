...
### 5.1 For Estimating a Mean

$$n = \left(\frac{z_{\alpha/2} \cdot \sigma}{\text{Margin of Error } E}\right)^2$$

**Example:** Desired half-width $E = \pm 2$ mm, $\sigma \approx 10$ mm, 95% confidence:
$$n = \left(\frac{1.96 \times 10}{2}\right)^2 = (9.8)^2 \approx 96$$

### 5.2 For Estimating a Proportion
...
### 6.1 Distribution of $\bar{x}$

If $X \sim \mathcal{N}(\mu, \sigma^2)$:

$$\bar{x} \sim \mathcal{N}(\mu, \sigma^2/n)$$

### 6.2 Distribution of $\hat{p}$
...
- $\text{SE}(\hat{x}) = s/\sqrt{n}$
- 95% CI: $\hat{x} \pm t_{0.025, n-1} \cdot s/\sqrt{n}$
