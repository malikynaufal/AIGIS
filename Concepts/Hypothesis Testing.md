...
- $Z \sim \mathcal{N}(0, 1)$ under $H_0$
- Use when $\sigma$ is known (large sample or known population variance)
...
- Use when $\sigma$ is estimated from the sample
...
- $t \sim t(n_1 + n_2 - 2)$ under $H_0$

**Welch's t-test** (unequal variances, more general):
...
$$H_0: \text{Data follows a specified distribution}$$

$$\chi^2 = \sum_{i=1}^k \frac{(O_i - E_i)^2}{E_i}$$

- $\chi^2 \sim \chi^2(k-1-m)$ (where $m$ = estimated parameters)
...
$$y_{ij} = \mu + \alpha_j + \epsilon_{ij}, \quad \epsilon_{ij} \sim \mathcal{N}(0, \sigma^2)$$
...
$$F = \frac{\text{MS}_{\text{between}}}{\text{MS}_{\text{within}}} = \frac{\frac{1}{k-1}\sum n_j(\bar{x}_j - \bar{x})^2}{\frac{1}{N-k}\sum\sum (x_{ij} - \bar{x}_j)^2}$$

- $F \sim F(k-1, N-k)$ under $H_0$

**ANOVA assumptions:** (1) Normality, (2) Homoscedasticity (equal variances), (3) Independence
