...
$$\mathbf{L} + \mathbf{V} = \mathbf{F}(\mathbf{X})$$
...
$$\mathbf{Q}_{\hat{\mathbf{V}}} = \sigma_0^2\bigl(\mathbf{P}^{-1} - \mathbf{A}\,\mathbf{N}^{-1}\,\mathbf{A}^\top\bigr)$$

For a derived quantity $y = \mathbf{c}^\top\hat{\mathbf{X}}$ (a single‑parameter function):
...
$$\hat{\mathbf{x}} = \begin{bmatrix} x_B \\ x_C \end{bmatrix} = \frac{1}{3}\begin{bmatrix} 2\,h_{AB} + h_{BC} - h_{AC} \\ h_{AB} + 2\,h_{BC} + h_{AC} \end{bmatrix}$$
