...
where $v$ = wave speed, $y(x,t)$ = displacement.
...
Path difference: $\Delta r = d\sin\theta$
- Bright fringes: $d\sin\theta = m\lambda$
- Dark fringes: $d\sin\theta = (m+\frac{1}{2})\lambda$

**Fringe spacing:** $\Delta y = \frac{\lambda L}{d}$ (small angle)
...
Minima: $a\sin\theta = m\lambda \quad (m = \pm1, \pm2, \dots)$

### Diffraction Grating
Multiple slits: $N$ slits of width $a$, separation $d$
...
where $\theta$ = angle between polarization direction and transmission axis.
...
### Example 1: Double Slit Fringes
$\lambda = 632.8$ nm, $d = 0.2$ mm, $L = 2$ m.
