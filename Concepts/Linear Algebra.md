...
3. Associativity, commutativity of addition
...
- **Linearly independent**: $c_1\mathbf{v}_1 + \cdots + c_n\mathbf{v}_n = \mathbf{0} \implies c_1 = \cdots = c_n = 0$
- **Span**: All linear combinations of $\{\mathbf{v}_1, \ldots, \mathbf{v}_n\}$
...
$|\det(A)|$ = factor by which $A$ scales $n$-dimensional volume.
...
For each $\lambda$, solve $(A - \lambda I)\mathbf{v} = \mathbf{0}$
...
- $A$ diagonalizable $\iff$ algebraic multiplicity = geometric multiplicity for all $\lambda$
...
- $A^TA = V\Sigma^TU^TV\Sigma V^T = V(\Sigma^T\Sigma)V^T$
- Rank = number of nonzero singular values
- Condition number $\kappa = \sigma_{\max}/\sigma_{\min}$

**Applications**: Pseudoinverse, data compression, noise filtering
...
### Problem 1
Find eigenvalues of $A = \begin{bmatrix} 2 & 1 \\ 1 & 2 \end{bmatrix}$

**Solution**:
$$\det(A-\lambda I) = (2-\lambda)^2 - 1 = \lambda^2 - 4\lambda + 3 = (\lambda-1)(\lambda-3) = 0$$
$\lambda_1 = 1$, $\lambda_2 = 3$

Eigenvectors: $\mathbf{v}_1 = (-1,1)^T$, $\mathbf{v}_2 = (1,1)^T$

### Problem 2
Compute LU decomposition of $A = \begin{bmatrix} 2 & 1 \\ 6 & 4 \end{bmatrix}$

**Solution**:
$R_2 \to R_2 - 3R_1$: $U = \begin{bmatrix} 2 & 1 \\ 0 & 1 \end{bmatrix}$, $L = \begin{bmatrix} 1 & 0 \\ 3 & 1 \end{bmatrix}$

Check: $LU = \begin{bmatrix} 2 & 1 \\ 6 & 4 \end{bmatrix}$ ✓

### Problem 3
Find the SVD of $A = \begin{bmatrix} 3 & 0 \\ 0 & 2 \end{bmatrix}$

**Solution**: Already diagonal, so $U = I$, $\Sigma = A$, $V = I$
