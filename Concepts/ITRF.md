...
$$\mathbf{X}_{ITRF_{new}} = \mathbf{T}(t) + (1+s(t))\,\mathbf{R}(t)\,\mathbf{X}_{ITRF_{old}}$$

where:
- $\mathbf{T}(t) = \mathbf{T}_0 + \dot{\mathbf{T}}\cdot(t - t_0)$
- $\mathbf{R}(t) = \mathbf{R}_0 + \dot{\mathbf{R}}\cdot(t - t_0)$
- $s(t) = s_0 + \dot{s}\cdot(t - t_0)$

### Transformation: ITRF2014 → ITRF2020 (at epoch 2015.0)
