...
- $\text{Heel Correction}$ = ship list correction: $\delta d = d \cdot \sin\theta$ for heel angle $\theta$

### 2.3 Corrections Applied to Soundings

1. **Heel correction** — for ship list
...
where:
- $D_{swath}$ = swath width (at depth $d$): $D_{swath} = 2d \cdot \tan(\alpha_{max})$
- $\Delta$ = overlap (typically 10–25%)
...
- **Pressure gauge** — measures hydrostatic pressure at seabed
