...
### 2.1. Code Pseudorange$$P_{r,f}^s = \rho_r^s + c(\delta t_r - \delta t^s) + I_{r,f}^s + T_r^s + d_{r,f}^s + d_f^s + \varepsilon_P$$### 2.2. Carrier Phase$$\Phi_{r,f}^s = \rho_r^s + c(\delta t_r - \delta t^s) - I_{r,f}^s + T_r^s + \lambda_f N_{r,f}^s + \delta_{r,f}^s + \delta_f^s + \varepsilon_\Phi$$| Term | Description | Frequency dependence |
|------|-------------|---------------------|
...
I_{r,f}^s = \frac{\alpha \cdot \text{STEC}_r^s}{f^2} \quad \text{where } \alpha = \frac{40.3}{c} \;\text{m}^3\text{s}^{-2}$$- **STEC** = Slant Total Electron Content (electrons/m² along the ray path).
...
### 5.1. The Integer Least Squares Problem$$\hat{\mathbf{a}}_{\text{int}} = \underset{\mathbf{a} \in \mathbb{Z}^n}{\arg\min} \; (\mathbf{a} - \hat{\mathbf{a}}_{\text{float}})^\top \mathbf{Q}_{\hat{\mathbf{a}}}^{-1} (\mathbf{a} - \hat{\mathbf{a}}_{\text{float}})$$### 5.2. LAMBDA Method
