...
) $\$ $\mathbf{X}_{2020}(t) = \mathbf{X}_{2014}(t) + \mathbf{T} + (1+s)\mathbf{R}\mathbf{X}_{2014}(t) + \Delta t\cdot(\dot{\mathbf{T}} + (1+\dot{s})\mathbf{R}\mathbf{X}_{2014}(t) + (1+s)\dot{\mathbf{R}}\mathbf{X}_{2014}(t)) $\$$**Practical steps**:
...
**Pseudorange**$ $ P^s = \rho^s + c(\delta t_r - \delta t^{s}) + I^s + T^s + \epsilon_P^s $ $**Carrier Phase**$ $\Phi^s = \rho^s + c(\delta t_r - \delta t^{s}) - I^s + T^s + \lambda N^s + \epsilon_\Phi^s
...
S $ $\text{PPP observation model:} \quad \mathbf{y} = \mathbf{A}\mathbf{x} + \mathbf{l} + \epsilon
...
### Integer Ambiguity Resolution (AR) in Multi-GNSS $ $ N_{\text{wide-lane}} = \frac{f_1 N_1 - f_2 N_2}{f_1 - f_2}N_{\text{narrow-lane}} = N_1 + N_{\text{wide-lane}} $$Modern PPP-AR enables cm-level accuracy with multi-GNSS.
...
### From ITRF to Local Surveys (Indonesia) $\$ $ \mathbf{X}_{\text{ITRF2020}}(t) = \mathbf{X}_{\text{ITRF2014}}(t_0) + \dot{\mathbf{X}}(t - t_0) + \Delta_{\text{Helmert}}H_{\text{TSSGI}} = h_{\text{ITRF2020}} - N_{\text{TSSGI}} $$
