...
### Fundamental Relationships$ $h = H + Nh = H^* + \zetaN - \zeta = \text{dynamic topography}$$where:
...
d$ $\Delta g_{res} = \Delta g_{obs} - g_{ref} $ $**Step 2 (Compute):** Compute$N$ from residual gravity using Stokes' integral

**Step 3 (Restore):** Add back the gravity from topography (Bouguer plate approximation
)$\$ $N = N_{res} + N_{topo}$$# ## Modern Hybrid Methods
...
GNSS gives ellipsoidal height $h$. Surveying needs orthometric height$H$:$ $H = h - N$ $**Example:** If$ h = 25.000 $m and$ N = 37.514 $m (EGM2008 at Jakarta)$\$ $H = 25.000 - 37.514 = -12.514\ \text{m}$$\(negative = below geoid!)
