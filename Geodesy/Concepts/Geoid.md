...
### Fundamental Relationships$$h = H + N
$$

$$h = H^* + \zeta$$

$$
N - \zeta = \text{dynamic topography}$$where:
...
### From Gravity Anomalies (Stokes' Integral)$$N_p = \frac{R}{4\pi\gamma} \int_\sigma \Delta g(q) S(\psi)\,d\sigma(q)$$where:
...
**Step 1 (Remove):** Subtract the ellipsoidal gravity field computed from a reference ellipsoid$$\Delta g_{res} = \Delta g_{obs} - g_{ref}$$**Step 2 (Compute):** Compute$N$from residual gravity using Stokes' integral

**Step 3 (Restore):** Add back the gravity from topography (Bouguer plate approximation)$$N = N_{res} + N_{topo}$$### Modern Hybrid Methods
...
H = h - N$$**Example:** If$h = 25.000$m and$N = 37.514$m (EGM2008 at Jakarta):$$H = 25.000 - 37.514 = -12.514\ \text{m}$$(negative = below geoid!)
