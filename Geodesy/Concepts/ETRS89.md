...
$$\mathbf{X}_{ITRF}(t) = \mathbf{X}_{ETRS89} + \mathbf{T}(t) + \mathbf{R}(t)\cdot\mathbf{X}_{ETRS89} + \mathbf{S}(t)$$where $\mathbf{T} $,$\mathbf{R} $,$\mathbf{S} $are the time-dependent translation, rotation, and scale parameters published by [[IERS]]/[[ITRF]].
