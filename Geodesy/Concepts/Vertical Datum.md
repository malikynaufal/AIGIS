...
$$h_N = h - \zeta$$where $\zeta $is the **height anomaly** (quasi-geoid undulation)
:
...
$$N = N_{long} + N_{res}N_{res} = \frac{G\Delta\rho}{\gamma} \int \int \frac{h - h_P}{r} \, d\sigma$$1. **Remove**: Remove long-wavelength geoid from satellite data (GRACE/GOCE)
...
$$\zeta = \frac{R}{4\pi\gamma_0} \int \int_{\sigma} \Delta g^* \cdot S(\psi) \, d\sigma + \zeta_0$$where $\Delta g^*$is the Molodenskii gravity anomaly and $\zeta_0 $is a constant determined from GNSS/levelling.
