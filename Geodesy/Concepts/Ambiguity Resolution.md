...
\Phi = \lambda N + \rho + c(dt_r - dt_s) - d_{iono} + d_{trop} + \varepsilon$$-$\Phi$= measured phase (observable)
...
## LAMBDA Method$$\hat{N}_{fixed} = \arg\min_{\mathbf{N} \in \mathbb{Z}^n} (\hat{N}_{float} - \mathbf{N})^T Q_N^{-1} (\hat{N}_{float} - \mathbf{N})$$1. Decorrelate with Z-transform
