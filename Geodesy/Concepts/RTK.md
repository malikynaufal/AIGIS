...
$$\Phi_{r,f}^s = \rho_r^s + c(\delta t_r - \delta t^s) + \lambda_f N_{r,f}^s + I_r^s + T_r^s + \epsilon_{\Phi}$$
...
$$\hat{\mathbf{a}}_{\text{float}} = (\mathbf{A}^\top\mathbf{P}\mathbf{A})^{-1}\mathbf{A}^\top\mathbf{P}\boldsymbol{\ell}$$
