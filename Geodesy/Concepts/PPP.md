...
The troposphere cannot be differenced out in PPP and must be estimated:$ $ T_r^s = m_h^s(\text{elev})\cdot Z_h + m_w^s(\text{elev})\cdot Z_w $$| Component | Mapping Function | Typical Value |
|-----------|------------------|---------------|
