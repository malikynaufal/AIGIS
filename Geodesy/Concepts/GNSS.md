...
\rho = c \cdot (t_{rx} - t_{tx}) = ||\mathbf{r}_{sat}(t) - \mathbf{r}_{rx}|| + c\Delta t$$**Resolving for 4 unknowns**$(x, y, z, \delta t)$requires ≥ 4 satellites:$$\begin{bmatrix} \rho_1 \\ \rho_2 \\ \rho_3 \\ \rho_4 \end{bmatrix} \xrightarrow{\text{least squares}} \begin{bmatrix} x_{rx} \\ y_{rx} \\ z_{rx} \\ \delta t_{rx} \end{bmatrix}$$### Observation Equations

**Code (pseudorange):**$$P_i = \rho + c(dt_r - dt_s) + d_{iono} + d_{trop} + \varepsilon_P$$**Carrier phase:**$$\Phi_i = \frac{\rho}{\lambda} + N_i + \frac{c}{\lambda}(dt_r - dt_s) - \frac{d_{iono}}{\lambda} + \frac{d_{trop}}{\lambda} + \varepsilon_\Phi$$where$\rho = ||\mathbf{r}_r - \mathbf{r}_s||$= geometric range,$N$= integer ambiguity.
...
Base station transmits corrections to rover:$$\rho_{corr} = \rho_{rover} + \Delta\rho_{base}$$**Accuracy:** 0.5–3 m (sub-meter with carrier smoothing)

### RTK (Real-Time Kinematic)

Float solution: estimate$N \in \mathbb{R}$Fixed solution: resolve$N \in \mathbb{Z}$(LAMBDA method)$$\hat{N}_{float} \xrightarrow{\text{LAMBDA}} \hat{N}_{fixed}$$**Accuracy:** 1–2 cm + 2 ppm (horizontal)
