...
where $\Delta t_0$ (proper time) is measured by clock at rest relative to events.

### Length Contraction
$$L = L_0/\gamma$$
where $L_0$ (proper length) measured in object's rest frame.
...
$$\nu_{\text{obs}} = \nu_{\text{source}} \sqrt{\frac{1 - \beta}{1 + \beta}} \quad \text{(approaching)}$$
...
1. $\gamma = 1 + K/(m_p c^2) = 1 + 7000/0.938 = 7463$
2. $v/c = \sqrt{1 - 1/\gamma^2} \approx 1 - 10^{-8}$ (99.999999% $c$)
