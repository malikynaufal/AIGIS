...
$$\vec{h} = \vec{r} \times \vec{v} = \text{constant}$$
...
### Kepler's Laws
1. **Elliptical orbits:** $r = \frac{a(1-e^2)}{1 + e\cos f}$
2. **Equal areas in equal times:** $\frac{dA}{dt} = \frac{h}{2} = \text{constant}$
3. **Period relation:** $T = 2\pi\sqrt{\frac{a^3}{GM}}$
...
### Transformation
$$\vec{r}_{ECEF} = R_z(\theta_G)\vec{r}_{ECI}$$
where $\theta_G$ = Greenwich sidereal time (GST).
...
$$\frac{da}{dt} = \frac{2}{n\sqrt{1-e^2}}\left[ e\sin f \cdot S + \frac{p}{r} \cdot T \right]$$
$$\frac{de}{dt} = \frac{\sqrt{1-e^2}}{na}\left[ \sin f \cdot S + \left( \cos f + \frac{e + \cos f}{1 + e\cos f} \right) \cdot T \right]$$
$$\frac{di}{dt} = \frac{r\cos(\omega+f)}{na\sqrt{1-e^2}} \cdot W$$
...
$$U = \frac{GM}{r}\left[1 - J_2\left(\frac{a_e}{r}\right)^2 P_2(\sin\phi) + J_3\left(\frac{a_e}{r}\right)^3 P_3(\sin\phi) + \cdots \right]$$

where $J_2 \approx 1.08263 \times 10^{-3}$, $a_e = 6,378,137$ m.
...
### Drag Acceleration
$$\vec{a}_D = -\frac{1}{2} \frac{C_D A}{m} \rho v^2 \hat{v}$$

where $C_D \approx 2.2$ (drag coefficient), $A/m$ = area-to-mass ratio, $\rho$ = atmospheric density.

### Density Models
- **Exponential model:** $\rho = \rho_0 \exp(-(h-h_0)/H)$
- **Scale height $H$:** ~50–100 km (varies with solar activity)
- **Models:** NRLMSISE-00, JB2008, DTM2020

### Orbital Decay
- Period change: $\frac{dT}{dt} \propto -\rho a^{1/2}$
- Lifetime at 400 km: ~months–years
...
### Acceleration
$$\vec{a}_{SRP} = -\frac{C_R A}{m} \frac{P_{solar}}{c} \left(\frac{1 \text{ AU}}{r}\right)^2 \hat{r}_{sun}$$

where $C_R \approx 1.2$–$1.8$ (reflectivity coefficient), $P_{solar} = 4.56 \times 10^{-6}$ N/m² at 1 AU.
...
### Secular Effects on Eccentricity Vector
$$\frac{d\vec{e}}{dt} \propto \frac{GM_3}{a_3^3} \frac{a^4}{GM} \left[ \text{periodic terms} \right]$$

For GPS:
- **Moon:** Causes ~0.2°/yr variation in $e$ and $\omega$
- **Sun:** Similar magnitude
...
- $M_0, \Delta n, e, \sqrt{a}, \Omega_0, i_0, \omega, \Omega_{dot}, i_{dot}$

Corrections (harmonic):
...
$$\frac{\Delta f_{SR}}{f_0} = -\frac{v^2}{2c^2}$$

GPS: **-7.2 μs/day** (clocks run slower due to orbital velocity)

### Net Effect
$$\frac{\Delta f_{net}}{f_0} = \frac{+45.8 - 7.2}{\text{day}} = +38.6 \mu\text{s/day}$$

### Factory Frequency Offset
GPS clocks set to: $f = 10.23 \text{ MHz} \times (1 - 4.465 \times 10^{-10}) = 10.22999999543 \text{ MHz}$

Once in orbit, relativistic speedup brings them into sync with ground UTC.
...
$$\Delta t_{Sagnac} = \frac{2\vec{\Omega}_e \cdot (\vec{r}_1 \times \vec{r}_2)}{c^2}$$
where $\vec{\Omega}_e$ = Earth rotation vector.
...
- **Predict:** $\hat{\vec{x}}_{k|k-1} = \Phi \hat{\vec{x}}_{k-1|k-1}$, $P_{k|k-1} = \Phi P_{k-1|k-1} \Phi^T + Q$
- **Update:** $K_k = P_{k|k-1}H^T(HP_{k|k-1}H^T + R)^{-1}$, $\hat{\vec{x}}_{k|k} = \hat{\vec{x}}_{k|k-1} + K_k(\vec{z}_k - H\hat{\vec{x}}_{k|k-1})$

Used in real-time orbit determination (e.g., GPS navigation solution, PPP).
