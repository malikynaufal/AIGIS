...
$$y = x_1^{a} x_2^{b} \implies \frac{\sigma_y}{|y|} \approx \sqrt{a^2\left(\frac{\sigma_{x_1}}{x_1}\right)^2 + b^2\left(\frac{\sigma_{x_2}}{x_2}\right)^2}$$

**Example:** Computing volume $V = \pi r^2 h$ with $r$ and $h$ measured:
...
**Advantage:** Adding a new point doesn't require recomputing the entire polynomial.
...
**Geodesy application:** Spline interpolation of geoid models, DEM surfaces, and trajectory reconstruction.

### 3.5 Chebyshev Polynomial Approximation
...
1. Compute $T(h), T(h/2), T(h/4), \ldots$
2. Extrapolate to $h \to 0$:

$$R_{i,j} = \frac{4^{j-1}R_{i,j-1} - R_{i-1,j-1}}{4^{j-1} - 1}$$

**Converges very fast for smooth functions.**
...
1. Compute integral on $[a, b]$ and on $[a, \frac{a+b}{2}] + [\frac{a+b}{2}, b]$
2. If difference exceeds tolerance, recursively subdivide
...
**Homogeneous solution** from characteristic equation $ar^2 + br + c = 0$:
...
**Applications in geodesy:** Satellite orbit propagation, inertial navigation computation, deformation modeling.
...
### 6.1 Scalar Linearisation

$$f(x) \approx f(x_0) + f'(x_0)(x - x_0)$$

### 6.2 Multivariable Linearisation
