...
where:
- $\mathbf{X}(t_0)$ = coordinates at reference epoch $t_0 = 2015.0$
- $\dot{\mathbf{X}}$ = site velocity (mm/year)
...
### PPP Processing with Multi-GNSS
...
$$N_{\text{wide-lane}} = \frac{f_1 N_1 - f_2 N_2}{f_1 - f_2}$$
$$N_{\text{narrow-lane}} = N_1 + N_{\text{wide-lane}}$$
...
$$\mathbf{X}_{\text{ITRF2020}}(t) = \mathbf{X}_{\text{ITRF2014}}(t_0) + \dot{\mathbf{X}}(t - t_0) + \Delta_{\text{Helmert}}$$
